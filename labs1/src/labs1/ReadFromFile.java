    ...
    public class ListNDG {
	              ...
	              for(int j=0; j<n; j++){
		        	  ...
		        	  while(node.next!=null)
			          {
			        	  if(node.next.ch.equals(vertexLists[j].ch))
			        	  {
			        		  if(!s[j])
				              {
				                   if(dist[u] + node.next.weight < dist[j])     //在通过新加入的u点路径找到离v0点更短的路径  
				                    {
	    ...
	    void queryBridgeWords()
	    {
	    	...
	    	if(count>1){
	    		...
	    		System.out.print("The bridge words from \""+word1+"\" to \""+word2+"\" are: "+"\""+qiaojieci[0]+"\"");
	    	}
	    }
	    ...
	    void randomWalk()throws IOException{
	    	...
	    	while(vertexLists[node].next!=null){   
                    ...
                    for(int j=0;j<i;j++){
                    	...
                    	if(q.ch.equals(qwe[j])&&q.next.ch.equals(qwe[j+1])){
                    ...
                    fileWriter.flush();
                    System.out.print(q.next.ch+" ");
                    node=chaxundanchibianhao(q.next.ch);
	    	}
	    	fileWriter.close();
	    }
    }   
